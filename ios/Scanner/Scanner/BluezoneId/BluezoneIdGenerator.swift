...
public class BluezoneIdGenerator {
     ...
     * Create BluezoneDailyKey
     */
    func createBluezoneDailyKey(dayStart: Int) -> BluezoneDailyKey {
        //var ret = nil
        let bluezoneDailyKey = getBluezoneBaseId();
        if bluezoneDailyKey.data.count > 0 {
            var bluezoneDailyKeyDx = bluezoneDailyKey.data
            
            // Call time
            let timeEnd = Double(Double(Date().currentTime()) - (Double(dayStart) * BluezoneIdConstants.DAY_MILLISECONDS))
            let times = Int(((timeEnd - bluezoneDailyKey.bluezoneDate.getTimeStart()) / BluezoneIdConstants.DAY_MILLISECONDS))
            var timeStart = bluezoneDailyKey.bluezoneDate.getTimeStart()
            ...
            return BluezoneDailyKey(data: bluezoneDailyKeyDx, bluezoneDate: BluezoneDate(date: Date(timeIntervalSince1970: timeStart)))
        }
        
        return BluezoneDailyKey(data: Data.init(), bluezoneDate: BluezoneDate())
    }
     ...
     */
    public static func convertBluezoneSubKeyToBluezoneId(_ bluezoneSubKey: Data) -> Data {
